...
END
  BEGIN TRANSACTION
  UPDATE TRIP
  SET [QUANTITY] = @p_QUANTITY,[QUANTITY_UNIT] = @p_QUANTITY_UNIT,[QUANTITY_TRIP] =@p_QUANTITY_TRIP,[GO_FROM] = @p_GO_FROM,[GO_TO]= @p_GO_TO,[TIME] = @p_TIME,[LENGTH] = @p_LENGTH,[LENGTH_UNIT] = @p_LENGTH_UNIT, [UNIT_ID] = @p_UNIT_ID, [UNIT_DISTANCE_ID] = @p_UNIT_DISTANCE_ID
...
AS
  ...
  VALUES (@p_AS_ID,@p_TYPE,@p_QUANTITY,@p_QUANTITY_UNIT,@p_QUANTITY_TRIP,@p_GO_FROM,@p_GO_TO,@p_TIME,@p_LENGTH,@p_LENGTH_UNIT, @p_UNIT_ID, @p_UNIT_DISTANCE_ID)
 IF @@Error <> 0 GOTO ABORT
  COMMIT TRANSACTION
